...
class object_properties_ugrid3D:
    '''
    The class initializes the set of properties required for detecting three
    dimensinal features from POP ugrid
    '''
    def check_coverage(self,data):
        '''
        The function checks whether the input data is global or regional
        '''
    ...
    def __init__(self,gridinfo,length,volume,smooth,
        ...
        '''
        Initializes the properties for the object
        '''
        ...
        # Calcualte the number of pixels correspinfing to minimum volume
        min_volume_pixel = int(volume*1e4/gridinfo['UAREA'].max().values)

        # Define simple interpolators for converting indexes to coordinates
...
#         ecc = [0.5,1.] if eccentricity==None else eccentricity
        ...
        # Define dictonary of object properties
...
class object_properties_tgrid:
    '''
    The class initializes the set of properties required for detecting two
    dimensinal features from POP tgrid
    '''
    def check_coverage(self,data):
...
#                                 gridinfo['DYT'].max()]))
        ...
        # Calcualte the number of pixels corresponding to minimum volume
        ...
        # Value for smooth scale
        smooth = self.min_length//3 if smooth==None else smooth
        # Coherence angle. Used only for vector quantities
        theta = 45 if theta_t==None else theta_t
        # Set shape index
        shapei = 0.375 if shapei==None else shapei
        # Set eccentricity for the detected object
        ecc = [0.5,1.] if eccentricity==None else eccentricity
...
class object_properties2D:
    '''
    The class initializes the set of properties required for detecting two
    dimensinal features from rectilinear grids
    '''
    ...
    def remove_holes(self,data):
        '''
        Function to remove small holes from land. This removes small water
        bodies like lakes
        '''
...
#                (min_area/cell_area.isel(lon=0)))
        ...
        # Define simple interpolators to calculate coordinate from indexes
        ...
        # Assign smoothing scale
        smooth = self.min_length//3 if smooth==None else smooth
        # Assign coherance angle
        theta = 45 if theta_t==None else theta_t
        # Assign shape index selection
        shapei = 0.375 if shapei==None else shapei
        # Assign eccentricity values for the object
        ecc = [0.5,1.] if eccentricity==None else eccentricity
