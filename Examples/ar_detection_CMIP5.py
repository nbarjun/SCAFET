...
# def extract_time(ds,timeslice=tslice):
#     return ds.sel(time=timeslice)
  
outfolder = '/Volumes/RAID5/Data/Atmosp_Rivers/ESGF/CCSM4/Output/'
expname = 'CCSM4'

folder = '/Volumes/RAID5/Data/Atmosp_Rivers/ESGF/CCSM4/'
grid_area = xr.open_mfdataset('{}areacella_fx_CCSM4_historical_r0i0p0.nc'.format(folder))\
                    ['areacella'].to_dataset(name='cell_area').load()
land_mask =  xr.open_mfdataset('{}sftlf_fx_CCSM4_historical_r0i0p0.nc'.format(folder))
...
datafolder  = '/Volumes/RAID5/Data/Atmosp_Rivers/ESGF/CCSM4'

files = {}
for v in var_names:
    if v == 'pr':
        files[v] = sorted(glob('{}/{}/{}_day_CCSM4_historical_*.nc'.format(datafolder,v,v)))[-2:]
    else:
        files[v] = sorted(glob('{}/{}/{}_day_CCSM4_historical_*.nc'.format(datafolder,v,v)))[-5:]

years = np.arange(1980,2005,5)
for yr in years:
    if yr==2000:
