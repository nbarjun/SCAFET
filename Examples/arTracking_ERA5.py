...
for exp in ['ERA5']:
    ...
    s=ar_props.time[0].values
    e=ar_props.time[-1].values

    ptime = xr.cftime_range(start=np.datetime_as_string(s)[:10],\
                end=np.datetime_as_string(e)[:10],freq='D')
    ar_objects['time']=ptime
    ar_props['time']=ptime   
