...
for v in var_names:
    if v == 'pr':
        files[v] = sorted(glob('{}/{}/{}_day_CCSM4_rcp45_*.nc'.format(datafolder,v,v)))[-1:]
    else:
        files[v] = sorted(glob('{}/{}/{}_day_CCSM4_rcp45_*.nc'.format(datafolder,v,v)))[-2:]
print(files)
years = np.arange(2095,2100,5)
for yr in years:
    if yr==2000:
