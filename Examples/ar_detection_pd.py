...
fnames={}
for exp,exn in zip(experiments[:1],exnames[:1]):
    fnames[exn]={}
    for vn in var_names:
...
# ivtfiles = sorted(glob(ivt_folder+'*.pd.*.nc'))[21:40]
ivtfiles = sorted(glob(ivt_folder+'*.pd.*.nc'))[20:40]

# tslice = slice('0140-12-21','0140-12-31')
pptfiles = fnames['PD']
outfolder = '/proj/arjun/CESM/Atmosp_Rivers/SCAFEX_V1/PD/'
